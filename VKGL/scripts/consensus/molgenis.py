...
'''
    response = self.session.get(
      self.url + "v1/" + quote_plus(entity) + '/' + quote_plus(id),
      headers=self._get_token_header(),
...
'''
    if q:
      ...
      response = self.session.get(self.url + "v2/" + quote_plus(entity),
                                  headers=self._get_token_header_with_content_type(),
                                  params={"q": q, "attributes": attributes,
                                          "expand": expand, "num": num,
                                          "start": start,
...
'''
  ...
  def update_one(self, entity, id, attr, value):
    ...
    response = self.session.put(
      self.url + "v1/" + quote_plus(entity) + "/" + id + "/" + attr,
      headers=self._get_token_header_with_content_type(),
  ...
  def add_all(self, entity, entities):
    ...
    print(response)
    if response.status_code == 201:
      return [resource["href"].split("/")[-1] for resource in
              response.json()["resources"]]
    else:
      errors = json.loads(response.content.decode("utf-8"))['errors'][0][
        'message']
      print(errors)
    response.raise_for_status()
    return response
  ...
  def delete(self, entity, id):
    ...
    response = self.session.delete(
      self.url + "v1/" + quote_plus(entity) + "/" + quote_plus(id), headers=
  ...
  def get_entity_meta_data(self, entity):
    ...
    response = self.session.get(
      self.url + "v1/" + quote_plus(entity) + "/meta?expand=attributes",
      headers=
      self._get_token_header())
    response.raise_for_status();
    return response.json();
  ...
  def get_attribute_meta_data(self, entity, attribute):
    ...
    response = self.session.get(
      self.url + "v1/" + quote_plus(entity) + "/meta/" + quote_plus(attribute),
      headers=
      self._get_token_header())
    response.raise_for_status();
    return response.json();
